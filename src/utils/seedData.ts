...
export const seedSampleDataToDatabase = async (): Promise<boolean> => {
  try {
    const sampleMemoData: MemoFormData[] = [
      {
        title: '프로젝트 회의 준비',
        content:
          '다음 주 월요일 오전 10시 프로젝트 킥오프 미팅을 위한 준비사항:\n\n- 프로젝트 범위 정의서 작성\n- 팀원별 역할 분담\n- 일정 계획 수립\n- 필요한 리소스 정리',
    ...
    for (const memoData of sampleMemoData) {
      await supabaseUtils.addMemo(memoData)
    }

    console.log('Sample data seeded to database successfully!')
    return true
  } catch (error) {
    console.error('Failed to seed sample data to database:', error)
    return false
  }
}
